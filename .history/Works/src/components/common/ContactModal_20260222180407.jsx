...
const impactCardPop = keyframes`
  ...
  50% {
    opacity: 1;
    transform: scale(0.95) translateY(10px); /* 쾅 하고 눌리는 느낌 */
    filter: brightness(1.5) blur(0px);
    box-shadow: 0 0 120px rgba(0, 242, 96, 1); /* 빛 폭발 */
  }
  75% {
    transform: scale(1.02) translateY(-5px); /* 반동 */
...
const profilePop = keyframes`
  0% {
    opacity: 0;
    transform: scale(0.3) translateY(30px); /* 아래에서 대기 */
  }
  50% {
    opacity: 1;
    transform: scale(1.15) translateY(-10px); /* 충격에 의해 위로 튕겨 오름 */
  }
  75% {
    transform: scale(0.95) translateY(5px); /* 착지 반동 */
  }
  100% {
    opacity: 1;
    transform: scale(1) translateY(0); /* 정착 */
  }
`;
